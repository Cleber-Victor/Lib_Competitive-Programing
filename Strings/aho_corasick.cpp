...
struct node { // pode adicionar um contador de prefixos
  ...
  int c;
  int idx = -1; // -1 indica que não é fim de palavra.
  int prf_cnt =0;                
};
...
void buildLinks(trie root) { // O(L) L-> numero de nós
  ...
  for(int i = 0; i < 26; i++) {
    if(root->nxt[i]) {
      q.push(root->nxt[i]);
      root->nxt[i]->sl = root; // Filhos da raiz têm sl para a raiz
    }
  }
  ...
  while (!q.empty()) {
    ...
    for (int c = 0; c < 26; c++) {
      if (t->nxt[c]) {
        ...
        // --- LÓGICA DO OUTPUT LINK---
        // Verifica se o nó do suffix link é o fim de uma palavra.
        // Se for (idx != -1), o output link aponta para ele.
        // Senão, herda o output link dele.
        if (son->sl->idx != -1) {
          son->ol = son->sl;
        } else {
          son->ol = son->sl->ol;
...
void search(trie root,const string& text, const vector<string>& patterns) { //O(M+Z) Custo para buscar em um texto de tamanho M com Z ocorrências.
  ...
  for (int i = 0; i < text.length(); ++i) {
    ...
    // 2. Lógica de transição: Se não houver caminho direto,
    ...
    // 3. Verificação de Padrões: Após a transição, verifica se há
    //    algum padrão terminando nesta posição.
    trie temp = current;
    while (temp != nullptr) {
      // Se o nó atual representa o fim de um padrão (idx != -1)...
      if (temp->idx != -1) {
      ...
      // ...pula para o próximo padrão na cadeia de sufixos usando o output link (ol).
      temp = temp->ol;
    }
  }
}
